...
case "$dpkgArch" in
	...
	arm64)
		if grep 'Model' /proc/cpuinfo | grep -q 'Raspberry Pi'; then
		    {
			echo
			echo 'WARNING: Using *unofficial* MongoDB community build for Raspberry Pi which was purpose-built to circumvent micro-architecture limitations.'
			echo '  Inadvisable for production environments. Caveat emptor.'
			echo '  see https://github.com/themattman/mongodb-raspberrypi-binaries'
			echo
		    } >&2
		    if grep 'Model' /proc/cpuinfo | grep -q 'Raspberry Pi 3'; then
			{
			    echo
			    echo '!WARNING! Running the *unofficial* MongoDB Pi community build on a Raspberry Pi 3 is venturing into uncharted territory.'
			    echo '  Inadvisable for production environments. Caveat emptor.'
			    echo '  This software should be considered to be running in lol-mode.'
			    echo '  This software is made available so that you can run it, not because you should.'
			    echo
			} >&2
		    fi
		elif ! grep -qE '^Features.* (fphp|dcpop|sha3|sm3|sm4|asimddp|sha512|sve)( .*|$)' /proc/cpuinfo; then
